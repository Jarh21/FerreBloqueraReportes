...
const ModalSolicitudPago: React.FC<ModalSolicitudPagoProps> = ({ 
  ...
  // ESTADOS LOCALES SIMPLES PARA EL RIF
  ...
  // 1. EFECTO SIMPLE: Unir Prefijo y Número cada vez que cambien
  useEffect(() => {
    // Simplemente unimos lo que haya, sin validar nada.
    // Si el usuario borra el número, mandamos cadena vacía o solo la letra, según prefieras.
    // Aquí asumimos que si hay número, unimos.
    const rifCompleto = rifNumero ? `${rifPrefijo}-${rifNumero}` : '';
    
    setFormData(prev => ({
        ...prev,
        beneficiario_rif: rifCompleto
    }));
  ...
  // --- AL SELECCIONAR DEL AUTOCOMPLETADO ---
  const handleBeneficiarioSeleccionado = (item: any) => {
      // Separar el RIF que viene de la BD para mostrarlo bonito en los inputs
      let p = 'V';
      let n = '';

      if (item.rif && item.rif.includes('-')) {
          const partes = item.rif.split('-');
          // Si la primera parte coincide con nuestros prefijos, la usamos
  ...
  // --- HELPER PARA RENDERIZAR RIF (YA NO ES UN COMPONENTE, ES UNA FUNCIÓN DIRECTA) ---
  // Esto soluciona el problema de pérdida de foco
      ...
      <div className="space-y-1">
          ...
          <div className="flex">
              <select
                  ...
                  className="bg-slate-100 border border-slate-200 text-sm font-bold text-slate-700 rounded-l-lg px-2 outline-none focus:bg-white focus:ring-2 focus:ring-red-700 transition-colors cursor-pointer w-16 text-center appearance-none"
              >
                  {PREFIJOS_RIF.map(p => <option key={p} value={p}>{p}</option>)}
              </select>
              <input
                  value={rifNumero}
                  onChange={(e) => setRifNumero(e.target.value)} // Sin regex, input libre
                  className={`w-full p-2.5 border border-l-0 border-slate-200 rounded-r-lg text-sm outline-none focus:ring-2 focus:ring-red-700 transition-all font-mono`}
  ...
  const renderCamposManuales = () => {
     switch (formData.tipo_pago) {
        case TIPOS_PAGO.EFECTIVO:
            return (
                <div className="space-y-1 md:col-span-3">
                    ...
                    <input name="beneficiario_nombre" value={formData.beneficiario_nombre} onChange={handleChange} className={inputClass} placeholder="Quien retira el efectivo..." required />
                <div className="space-y-1 "><label className={labelClass}>Concepto / Nota</label><input name="concepto" value={formData.concepto} onChange={handleChange} className={inputClass} /></div>
                </div>
            );
        case TIPOS_PAGO.ZELLE:
            return (
                <>
                <div className="space-y-1 md:col-span-2"><label className={labelClass}>Correo Zelle</label><input name="beneficiario_email" value={formData.beneficiario_email} onChange={handleChange} className={inputClass} required/></div>
                <div className="space-y-1"><label className={labelClass}>Titular</label><input name="beneficiario_nombre" value={formData.beneficiario_nombre} onChange={handleChange} className={inputClass} required/></div>
        ...
        case TIPOS_PAGO.BINANCE:
            return (
              <>
                <div className="space-y-1 md:col-span-2"><label className={labelClass}>Binance ID / Email</label><input name="beneficiario_email" value={formData.beneficiario_email} onChange={handleChange} className={inputClass} placeholder="ID o Correo" required/></div>
                 <div className="space-y-1"><label className={labelClass}>Nombre Usuario</label><input name="beneficiario_nombre" value={formData.beneficiario_nombre} onChange={handleChange} className={inputClass} required /></div>
                  <div className="space-y-1"><label className={labelClass}>Concepto</label><input name="concepto" value={formData.concepto} onChange={handleChange} className={inputClass} /></div> 
              </>
            );
        case TIPOS_PAGO.PAGO_MOVIL:
            return (
                ...
                {/* Renderizamos el RIF llamando a la función helper (NO COMO COMPONENTE) */}
                {renderRifInput()}
                
                <div className="space-y-1"><label className={labelClass}>Teléfono</label><input name="beneficiario_telefono" value={formData.beneficiario_telefono} onChange={handleChange} className={inputClass}required /></div>
                 <div className="space-y-1"><label className={labelClass}>Concepto</label><input name="concepto" value={formData.concepto} onChange={handleChange} className={inputClass} /></div> 
                </>
            );
        case TIPOS_PAGO.TRANSFERENCIA:
            return (
                <>
                <div className="space-y-1 md:col-span-2"><label className={labelClass}>Beneficiario</label><input name="beneficiario_nombre" value={formData.beneficiario_nombre} onChange={handleChange} className={inputClass} required /></div>
                
                {/* Renderizamos el RIF llamando a la función helper */}
                {renderRifInput()}
                
                <div className="space-y-1"><label className={labelClass}>Banco</label><InputBancosAutocomplete name="beneficiario_banco" value={formData.beneficiario_banco} onChange={handleChange} className={inputClass} /></div>
                <div className="space-y-1 md:col-span-2"><label className={labelClass}>Nro Cuenta</label><input name="beneficiario_cuenta" value={formData.beneficiario_cuenta} maxLength={20} onChange={handleChange} className={inputClass}required /></div>
                 <div className="space-y-1"><label className={labelClass}>Concepto</label><input name="concepto" value={formData.concepto} onChange={handleChange} className={inputClass} /></div> 
                </>
            );
