...
const ModalProcesarPago: React.FC<ModalProcesarPagoProps> = ({ 
    ...
    // Variable auxiliar para saber si deshabilitamos la tasa
    ...
    useEffect(() => {
        if (isOpen && solicitud) {
            ...
            // Si es USD, la tasa es irrelevante (o es 1), pero la mostramos bloqueada
            const tasaRegistrada = solicitud.tasa_cambio || '';
            setTasaAplicada(tasaRegistrada);
            setOrigenTasa('REGISTRADA');

            // Consultar API solo si es VES
            if (!esDolar) {
                fetch('https://ve.dolarapi.com/v1/dolares/oficial')
                    .then(res => res.json())
                    .then(data => {
                        if (data && data.promedio) {
                            setTasaApi(parseFloat(data.promedio));
                        }
                    })
                    .catch(console.error);
            }
    ...
    const handleCambioOrigenTasa = (nuevoOrigen: 'REGISTRADA' | 'ACTUAL' | 'MANUAL') => {
        ...
        if (nuevoOrigen === 'REGISTRADA') {
            setTasaAplicada(solicitud.tasa_cambio || '');
        } else if (nuevoOrigen === 'ACTUAL') {
            setTasaAplicada(tasaApi.toString());
        }
    };

    // --- MANEJO DE IMAGEN (Paste) ---
    useEffect(() => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!bancoOrigen || !referencia) {
            toast.error("Faltan datos", { description: "Banco y Referencia son obligatorios." });
            return;
        }
        if (montoFinal <= 0) {
        ...
        if (modoPago === 'abono' && montoFinal > deudaPendiente + 0.01) { 
            toast.warning("Monto Excedido", { description: "El abono supera la deuda." });
            return;
        }
        ...
        const datosPago = {
            ...
            // Si es dolar, mandamos null o 1, sino la tasa aplicada
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col">
                ...
                <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                   ...
                   <div className="md:col-span-2">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 opacity-90 p-2 mb-0.5 bg-green-100 rounded-lg">
                            ...
                            <div className="md:col-span-1">
                                <label className={labelClass}>Ref. original Valor ($)</label>
                                <input value={solicitud.referencia_usd ? `$${solicitud.referencia_usd}` : '-'} disabled className={inputDisabledClass} />
                            </div>
                            
                            <div className="md:col-span-1">
                                <label className={labelClass}>Tasa Original</label>
                                <input value={solicitud.tasa_cambio || '-'} disabled className={inputDisabledClass} />
                            </div>

                            </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 opacity-90 p-4 bg-slate-50 rounded-lg">
                            ...
                            <div className="md:col-span-2">
                                <label className={labelClass}>Cuenta / Detalle destino</label>
                                <input value={`${(solicitud.beneficiario_banco || '').slice(0,35)}  / ${solicitud.beneficiario_identificador || ''}`} disabled 
          ...
          <div className="md:col-span-2"><label className={labelClass}>Descripcion</label><input value={solicitud.concepto || ''} disabled className={inputDisabledClass} /></div>
                    ...
                    <div className="md:col-span-2 mt-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 p-4 bg-green-50/30 rounded-xl border border-green-100">
                            ...
                            {/* --- SELECTOR DE TASA + INPUT (CON LÓGICA DE BLOQUEO SI ES USD) --- */}
                            <div>
                                <label className={labelClass}>Tasa de Cambio</label>
                                {esDolar ? (
                                    // Si es Dólar, mostramos un input deshabilitado simple o un mensaje
                                    <div className="relative">
                                        <input value="N/A (Pago en Divisa)" disabled className={inputDisabledClass} />
                                    </div>
                                ) : (
                                    // Si es Bolívares, mostramos el selector completo
                                    <div className="flex">
                                        <select 
                                            value={origenTasa} 
                                            onChange={(e) => handleCambioOrigenTasa(e.target.value as any)} 
                                            className="bg-green-100 border border-green-200 text-xs font-bold text-green-800 rounded-l-lg px-2 outline-none focus:bg-white transition-colors cursor-pointer w-28"
                                        >
                                        ...
                                        <input 
                                            ...
                                            className={`w-full p-2.5 border border-l-0 border-green-200 rounded-r-lg text-sm outline-none transition-all ${origenTasa !== 'MANUAL' ? 'bg-slate-50 text-slate-500 cursor-not-allowed' : 'bg-white text-slate-800 focus:ring-2 focus:ring-green-600'}`} 
                                            placeholder="0.00"
                                        />
                                    </div>
                                )}
                                {/* Info auxiliar */}
                                {origenTasa === 'ACTUAL' && !esDolar && (
                                    <p className="text-[9px] text-green-600 mt-1 ml-1">Tasa BCV del día: {tasaApi}</p>
                                )}
                            </div>
                            {/* ----------------------------------------------------------------- */}
                            ...
                            <div className="md:col-span-2">
                                <div className="flex justify-between items-center mb-1">
                                    ...
                                    <button type="button" onClick={handlePasteImage} className="text-[10px] font-bold text-blue-600 hover:text-blue-800 flex items-center gap-1 bg-blue-50 px-2 py-0.5 rounded cursor-pointer transition-colors">
                                        Pegar (Ctrl+V)
                                    </button>
                                </div>
